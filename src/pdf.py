...
def save_pdf_to_pngs(hashed_name):
    page_urls = []
    pdf_file = '{}/{}'.format(config.MEDIA_PDF, hashed_name)
    try:
...
def save_pdf_file(body, pdf_name, user_name):
    ...
    file_path = '{}/{}'.format(config.MEDIA_PDF, hashed_name)
...
def save_file(body, name, user_name):
    hashed_name = hashlib.md5(str(time.time()).encode()).hexdigest()
    file_path = '{}/{}'.format(config.MEDIA_PDF, hashed_name)
    with open(file_path, 'wb') as f:
        f.write(body)
    db.insert_file(name, hashed_name, user_name)
    logging.debug('save_file: {} saved ({} bytes) in {}'.format(pdf_name, len(body), hashed_name)) # noqa
